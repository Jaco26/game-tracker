...
def count(cities):
  ...
  def exclude_ids_from(*args):
    """ Accepts arguments which are of type 'list' or 'dict'. A 'dict' arg must
    contain a top-level 'id' key. Likewise, a 'list' arg must
    contain 'dict's which have an 'id' key. It will return a list of <int 'id'>
    """
    ids_to_exclude = []
    for arg in args:
      if type(arg) is dict:
